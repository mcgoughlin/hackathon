...
class UNet_featureoutput(nn.Module):
    ...
    def forward(self, xb):
        ...
        return logs_list[::-1], feature_list[::-1]
...
class UNet_ClassHead_MaxPool_featureoutput(nn.Module):
    ...
    def forward(self, xb):
        ...
        out_class_latents = torch.cat(class_latents, dim=1).to(xb.device)

        out = self.final_fc2(self.ReLU(self.final_fc(out_class_latents)))

        if self.stem_kernel_size is not None:
            logs_list.append(self.ReLU(self.final_up_conv(xb)))

        # as we iterate from bottom to top we have to flip the logits list
        return logs_list[::-1], out, feature_list[::-1], out_class_latents
...
class UNet_ClassHead_PowerPool_featureoutput(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_sizes,
        ...
        for count, (in_channels, out_channels, hid_channels, kernel_size) in enumerate(zip(self.in_channels_up_list,
            ...
            # print("in out sp",in_channels, out_channels, decoder_sp_size)
            self.class_pooling.append(GlobalPowerPooling())
            self.fcs.append(nn.Linear(out_channels, out_channels))
            final_fc_size += out_channels
...
# this class wraps a UNet_ClassHead_MaxPool_featureoutput and adds a regression head, based on forward pass feature list
# provided by the UNet_ClassHead_MaxPool_featureoutput
class UNet_regressivehead_wrapper(nn.Module):
    def __init__(self,UNet,depth,start_filters=32,patch_size=64):
        super().__init__()
        self.UNet = UNet
        self.depth = depth
        self.num_upsamplers = self.depth - 2

        self.upsamplers = []
        #kernel size needs to increase by 2 for each upsampling layer

        for i in range(self.num_upsamplers):
            self.upsamplers.append(UpConv(start_filters*(2**i),
                                          3,
                                          is_2d=False,
                                          kernel_size=2**i))

        self.upsamplers = nn.ModuleList(self.upsamplers)
        #create a conv layer with 3*self.num_upsamplers input channels and 1 output channel
        self.regressive_head = ConvNormNonlinBlock(in_channels=3*self.num_upsamplers,
                                                    out_channels=1,
                                                    is_2d=False,
                                                    kernel_size=3)

    def forward(self,xb):
        logs_list, out, feature_list, out_class_latents = self.UNet(xb)
        regressives = []
        for i in range(self.num_upsamplers):
            regressives.append( self.upsamplers[i](feature_list[i]))

        # channell-wise concat, then apply the regressive head
        regressive = torch.cat(regressives,dim=1)
        regressive = self.regressive_head(regressive)

        return logs_list, out_class_latents, regressive


class UNet_regressivehead_wrapper_kidney(nn.Module):
    def __init__(self,UNet,depth,start_filters=32,patch_size=64):
        super().__init__()
        self.UNet = UNet
        self.depth = depth
    ...
    def forward(self,xb):
        logs_list, feature_list = self.UNet(xb)
        regressives = []
        for i in range(self.num_upsamplers):
            regressives.append( self.upsamplers[i](feature_list[i]))
...
if __name__ == '__main__':
    ...
    patch = 64
    kernel_sizes = [3 for _ in range(6)]
    net_3d =UNet_ClassHead_PowerPool_featureoutput(1, 2, kernel_sizes,
                                                 False, filters_max=1024, filters=32 )

    wrapper = UNet_regressivehead_wrapper(net_3d,6)
    print(wrapper)
    xb_3d = torch.randn((2, 1, patch, patch, patch), device=gpu)
    print('3d')
    with torch.no_grad():
        logs_list, out_class_latents, regressive = wrapper(xb_3d)
    print('Output shapes seg:')
    for log in logs_list:
        print(log.shape)

    print('Output shapes features:')
    for log in out_class_latents:
        print(log.shape)

    print('Output shapes cls_features:',out_class_latents.shape)
    print('Output shapes reg:',regressive.shape)
    print('Output classification:',logs_list)