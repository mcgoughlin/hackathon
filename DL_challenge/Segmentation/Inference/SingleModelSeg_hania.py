...
class Ensemble_Seg(nn.Module):
    def __init__(self, data_name: str = None,name='',  ##seg preprocess args
                 seg_fp: str = None, spacing=np.array([3, 3, 3]),
                 do_prep=False, do_infer=False,cont=False,
                 patch_size = 64, batch_size=4,overlap=0.5):  ##seg args
        super().__init__()

        print("")
        print("Initialising Segmentation System...")
        print("")
        ...
        # ### SEG PREPROCESS ###
        self.preprocessed_name = name+'_'+str(spacing[0]) + ',' + str(spacing[1]) + ',' + str(spacing[2]) + "mm"
        ...
        self.lrsv_fold_size = join(lrsv_fold, name+"_{}mm".format(self.spacing))
        if not exists(self.lrsv_fold_size):
            mkdir(self.lrsv_fold_size)

        self.sv_fold_size = join(self.seg_save_loc, self.data_name, name+'_{}mm'.format(self.spacing))
        if not exists(self.sv_fold_size):
            mkdir(self.sv_fold_size)

        if cont:
            self.lrsv_fold_size_c = join(lrsv_fold, name+"_{}mm_cont".format(self.spacing))
            if not exists(self.lrsv_fold_size_c):
                mkdir(self.lrsv_fold_size_c)

            self.sv_fold_size_c = join(self.seg_save_loc, self.data_name, name+'_{}mm_cont'.format(self.spacing))
    ...
    def Segmentation_Preparation(self, seg_spacing,
        ...
        preprocessing = SegmentationPreprocessing(apply_resizing=True,
                                                  ...
                                                  pooling_stride=None,
                                                  window=np.array([-200, 300]),
                                                  scaling=np.array([100, 0]),
                                                  lb_classes=None,
    ...
    def _load_UNet(self, path=None):
        ...
        for foldpath in model_files:
            self.SegModel = SegLoader(1, 2, 7, 2, filters=32, filters_max=1024)
    ...
    def seg_pred(self, data_tpl, do_postprocessing=True):
        ...
        pred_holder = np.where(pred_holder > len(self.Segment)//2, 1, 0)
        print("pred_holder max", pred_holder.max())
        return pred_holder, np.where(pred_lowres > len(self.Segment)//2, 1, 0)
    ...
    def seg_pred_cont(self, data_tpl):
        ...
        return pred_holder/len(self.Segment), pred_lowres/len(self.Segment)
    ...
    def Segment_CT(self,volume_thresholds=[250],cont=False):
        ...
        self.Segment = []
        self._load_UNet(self.seg_mp_low)
        ...
        params_low['n_folds']=5

        for i in range(params_low['n_folds']):
