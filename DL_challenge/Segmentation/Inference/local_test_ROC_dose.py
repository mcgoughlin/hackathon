...
for conf in confidence_thresholds:
    ...
    for file in [path for path in os.listdir(cancer_infp) if path.endswith('.nii.gz')]:
        ...
        #dilate kidney inferences by 10 voxels using binary dilation
        ...
        for region in regionprops(spim.label(cancer_voxels == 2)[0]):
            # print(f'Cancer region: {region.area} voxels')
            #print the proportion of the centroid to the image shape to 2dp
            # print(f'Centroid: {np.round( [region.centroid[i]/cancer_voxels.shape[i] for i in range(3)],2)}\n')
        ...
        #print kidney centroids and kidney shapes
        ...
        for pos,lb,region in zip(positions,ordered_positions,regionprops(spim.label(kidney_inf == 1)[0])):
            ...
            # print(pos,lb,prediction_region.max())
...
path = '/Users/mcgoug01/Downloads/test_results'
