...
class Ensemble_Seg(nn.Module):
    def __init__(self, data_name: str = None,  ##seg preprocess args
        ...
        # ### SEG PREPROCESS ###
        self.preprocessed_name = str(spacing[0]) + ',' + str(spacing[1]) + ',' + str(spacing[2]) + "mm_AA_"+str(is_cect)
        ...
        self.lrsv_fold_size = join(lrsv_fold, self.preprocessed_name)
        if not exists(self.lrsv_fold_size):
            mkdir(self.lrsv_fold_size)

        self.sv_fold_size = join(self.seg_save_loc, self.data_name, self.preprocessed_name)
        if not exists(self.sv_fold_size):
            mkdir(self.sv_fold_size)

        if cont:
            self.lrsv_fold_size_c = join(lrsv_fold, self.preprocessed_name+"_cont")
            if not exists(self.lrsv_fold_size_c):
                mkdir(self.lrsv_fold_size_c)

            self.sv_fold_size_c = join(self.seg_save_loc, self.data_name,  self.preprocessed_name+"_cont")
    ...
    def Segmentation_Preparation(self, seg_spacing,
        ...
        if self.is_cect:
            preprocessing = SegmentationPreprocessing(apply_resizing=True,
                                                      ...
                                                      pooling_stride=None,
                                                      window=np.array([-278, 279]),
                                                      scaling=np.array([55.6, 68]),
                                                      lb_classes=None,
    ...
    def _load_UNet(self, path=None,
        ...
        for foldpath in model_files:
            self.SegModel = SegLoader(1, 10, 6, 2, filters=32, filters_max=1024)
    ...
    def seg_pred(self, data_tpl, do_postprocessing=True):
        ...
        for model in self.Segment:
            ...
            if type(pred_holder) == type(None):
                pred_holder = np.where((data_tpl['pred_orig_shape']==4) | (data_tpl['pred_orig_shape']==5), 1, 0)
                pred_lowres = np.where((data_tpl['pred']==4) | (data_tpl['pred']==5), 1, 0)
            else:
                pred_holder += np.where((data_tpl['pred_orig_shape']==4) | (data_tpl['pred_orig_shape']==5), 1, 0)
                pred_lowres += np.where((data_tpl['pred']==4) | (data_tpl['pred']==5), 1, 0)
    ...
    def seg_pred_cont(self, data_tpl):
        ...
        for model in self.Segment:
            ...
            if type(pred_holder) == type(None):
                pred_holder = np.max(data_tpl['pred_cont_orig_shape'][4:6],axis=0)
                pred_lowres = np.max(data_tpl['pred_cont'][4:6],axis=0)
            else:
                pred_holder += np.max(data_tpl['pred_cont_orig_shape'][4:6],axis=0)
                pred_lowres += np.max(data_tpl['pred_cont'][4:6],axis=0)
    ...
    def Segment_CT(self,volume_thresholds=[250],cont=False):
        ...
        self.SegProcess = SegProcess(apply_small_component_removing=False, lb_classes=[1,2,3,4,5,6,7,8,9],
