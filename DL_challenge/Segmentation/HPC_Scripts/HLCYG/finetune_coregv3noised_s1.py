...
data_name = 'coreg_v3_HLCYG_'+str(dose)
spacing = 4
...
for vf in [0,1,2,3,4]:
    pretrain_loc = '/bask/projects/p/phwq4930-renal-canc/data/seg_data/trained_models/abdominal_atlas/4mm/6layer/fold_0/network_weights'
    model = SegmentationModel(val_fold=vf,
                              data_name=data_name,
                              preprocessed_name=preprocessed_name,
                              model_name=model_name,
                              model_parameters=model_params)
    nw = torch.load(pretrain_loc, map_location=dev)
    #remove keys with 'logits' in them
    for k in list(nw.keys()):
        if 'logits' in k:
            del nw[k]
    model.network.load_state_dict(nw, strict=False)
