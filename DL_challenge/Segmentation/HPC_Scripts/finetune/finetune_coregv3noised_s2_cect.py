...
data_name = 'masked_coreg_cect_v3'
spacing = 2
fold = int(sys.argv[1])
efr = 1000
dfr = 100
...
for vf in vfs:
    path_to_model = '/bask/projects/p/phwq4930-renal-canc/data/seg_data/trained_models/kits23_add_nooverlap/2mm_binary_canceronly/6,3x3x3,32_justcancer_trainontest/fold_0/network_weights'
    ...
    loaded = torch.load(path_to_model, map_location=dev)
    ...
    # load in pretrained weights into model.network where the keys match
    model.network.load_state_dict(loaded, strict=False)
