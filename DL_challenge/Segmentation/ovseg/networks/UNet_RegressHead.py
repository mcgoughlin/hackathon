...
class Regression(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, is_2d, p_dropout=0):
        super().__init__()
        if is_2d:
            self.rgr = nn.Conv2d(in_channels, out_channels, 1, bias=False)
            self.dropout = nn.Dropout2d(p_dropout, inplace=True)
        else:
            self.rgr = nn.Conv3d(in_channels, out_channels, 1, bias=False)
            self.dropout = nn.Dropout3d(p_dropout, inplace=True)
        nn.init.kaiming_normal_(self.rgr.weight)

    def forward(self, xb):
        return self.dropout(self.rgr(xb))
...
# %%
class UNet_RegressHead(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_sizes,
        ...
        for in_channels in self.logits_in_list:
            ...
            self.all_regress.append(Regression(in_channels=in_channels,
                                          out_channels=1,
                                          is_2d=self.is_2d,
                                          p_dropout=self.p_dropout_logits))
        ...
        self.all_regress = nn.ModuleList(self.all_regress)
        ...
        if self.stem_kernel_size is not None:
            ...
            self.final_up_reg = UpConv(self.filters,
                                        1,
                                        self.is_2d,
                                        self.stem_kernel_size)
    ...
    def forward(self, xb):
        ...
        for i in range(self.n_pyramid_scales - 1, -1, -1):
            ...
            regs = self.all_regress[i](xb)
            logs_list.append(logs), regs_list.append(regs)

        if self.stem_kernel_size is not None:
            logs_list.append(self.final_up_conv(xb))
            regs_list.append(self.final_up_reg(xb))


        # as we iterate from bottom to top we have to flip the logits list
        return logs_list[::-1], regs_list[::-1]
...
def get_2d_UNet(in_channels, out_channels, n_stages, filters=32):
    ...
    return UNet_RegressHead(in_channels, out_channels, kernel_sizes, True)


def get_3d_UNet(in_channels, out_channels, n_stages, n_2d_blocks, filters=32):
    kernel_sizes = [(3,3,3) if i < n_2d_blocks else 3
                    for i in range(n_stages)]
    return UNet_RegressHead(in_channels, out_channels, kernel_sizes, False, filters=filters)


# %%
if __name__ == '__main__':
    gpu = torch.device('cpu')
    net_3d = get_3d_UNet(1, 2, 6, 2, 8).to(gpu)
    xb_3d = torch.randn((1, 1, 64, 64, 64), device=gpu)
    print('3d')
    with torch.no_grad():
        logs,regs = net_3d(xb_3d)
    print('Output shapes:')
    for log,reg in zip(logs,regs):
        print(log.shape,reg.shape)
