...
# import utils
import KCD.Segmentation.ovseg.networks.swin_utils as utils
from KCD.Segmentation.ovseg.networks.swin_utils import trunc_normal_
...
class iBOTHead_w_Cls_Token_Old(DINOHead):

    def __init__(self, *args, patch_out_dim=8192, norm=None, act='gelu', last_norm=None,
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, norm_last_layer=True,
                 shared_head=False, **kwargs):

        super(iBOTHead_w_Cls_Token_Old, self).__init__(*args,
                                                       norm=norm,
                                                       act=act,
                                                       last_norm=last_norm,
                                                       nlayers=nlayers,
    ...
    def forward(self, x):
        if len(x.shape) == 2:
            return super(iBOTHead, self).forward(x)
        print('info: input head x size is ', x.shape)
        if self.last_layer is not None:
            x = self.mlp(x)
            x = nn.functional.normalize(x, dim=-1, p=2)
            x1 = self.last_layer(x[:, 0])
...
class iBOTHead_w_Cls_Token(DINOHead):

    def __init__(self, *args, patch_out_dim=8192, norm=None, act='gelu', last_norm=None,
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, norm_last_layer=True,
                 shared_head=False, **kwargs):

        super(iBOTHead_w_Cls_Token, self).__init__(*args,
                                                   norm=norm,
                                                   act=act,
                                                   last_norm=last_norm,
                                                   nlayers=nlayers,
        ...
        if not shared_head:
            if bottleneck_dim > 0:
                ...
                if norm_last_layer:
                    self.last_layer2.weight_g.requires_grad = False
                print('info: last layer 2 not not equle to 1!!!!!!!!!!!!!!!!!!!!!!!!!')
            else:
                self.mlp2 = nn.Linear(hidden_dim, patch_out_dim)
                self.last_layer2 = None
...
class iBOTHead_for_one_Token(DINOHead):

    def __init__(self, *args, patch_out_dim=8192, norm=None, act='gelu', last_norm=None,
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, norm_last_layer=True,
                 shared_head=True, **kwargs):

        super(iBOTHead_for_one_Token, self).__init__(*args,
                                                     norm=norm,
                                                     act=act,
                                                     last_norm=last_norm,
                                                     nlayers=nlayers,
