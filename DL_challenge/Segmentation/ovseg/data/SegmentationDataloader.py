...
class SegmentationBatchDataset_regress(object):

    def __init__(self, vol_ds, patch_size, batch_size, regress_key = 'noises',epoch_len=250, p_bias_sampling=0,
                 min_biased_samples=1, augmentation=None, padded_patch_size=None,
                 n_im_channels: int = 1, store_coords_in_ram=True, memmap='r', image_key='image',
                 label_key='label', store_data_in_ram=False, return_fp16=True, n_max_volumes=None,
                 bias='fg', n_fg_classes=None, *args, **kwargs):
    ...
    def _get_volume_tuple(self, ind=None):
        ...
        if load_from_ram:
            ...
            im = np.load(path_dict[self.image_key], 'r')
            labels = np.load(path_dict[self.label_key], 'r')
            regr_data = np.load(path_dict[self.image_key].replace('images',self.regress_key), 'r')
            og_ncct = im - regr_data
            volumes = [im, og_ncct, labels]
...
def SegmentationDataloader_regress(vol_ds, patch_size, batch_size, num_workers=None, regress_key = 'noises',
                           pin_memory=True, epoch_len=250, *args, **kwargs):
    dataset = SegmentationBatchDataset_regress(vol_ds=vol_ds, patch_size=patch_size, batch_size=batch_size,
                                       regress_key = regress_key,epoch_len=epoch_len, *args, **kwargs)
