...
class NetworkTraining(TrainingBase):
    ...
    def __init__(self, network, trn_dl,  model_path, loss_params={},
                 ...
                 augmentation=None, val_dl=None, nu_ema_trn=0.99,
                 ...
                 is_voxsim=None,decoder_frozen = 0,encoder_frozen = 0):
        ...
        assert self.lr_schedule in ['almost_linear', 'lin_ascent_cos_decay','lin_ascent_log_decay']
        ...
        self.decoder_frozen = decoder_frozen
    ...
    def update_lr(self, step=0):
        ...
        elif self.lr_schedule == 'lin_ascent_log_decay':
            n_warm = self.lr_params['n_warmup_epochs']
            lr_max = self.lr_params['lr_max']
            if self.epochs_done < n_warm:
                lr = lr_max * (step + 1 + self.epochs_done * len(self.trn_dl)) \
                ...
                lr = lr_max * np.exp(-self.lr_exponent * (self.epochs_done - n_warm) / (self.num_epochs - n_warm))
            self.opt.param_groups[0]['lr'] = lr
            if step == -1:
                self.print_and_log('Learning rate now: {:.4e}'.format(lr))
    ...
    def do_trn_step(self, batch, step, epochs_done):
        ...
        if self.fp32:
            loss = self.compute_batch_loss(batch, epochs_done)
            loss.backward()
            self.opt.step()
        else:
            with amp.autocast():
            ...
            with torch.no_grad():
                if self.fp32:
                    for batch in self.val_dl:
                        loss = self.compute_batch_loss(batch,epochs_done=-1)
                        val_loss += loss.item()
                else:
