...
ld_conf_per_patient_fp = '/Users/mcgoug01/Cambridge University Dropbox/William McGough/AIvsRad/AI/predictions_v3/lowdose_test_ROCv3/percase_confidences.csv'
fd_conf_per_patient_fp = '/Users/mcgoug01/Cambridge University Dropbox/William McGough/AIvsRad/AI/predictions_v3/fulldose_test_ROCv3/percase_confidences.csv'

# load confidences
ld_conf_per_patient = pd.read_csv(ld_conf_per_patient_fp)[['RightCancer', 'LeftCancer','RLabel', 'LLabel']]
#confidences are the unique vaues in RightCancer and LeftCancer
confidences = np.unique(ld_conf_per_patient['RightCancer'].values)
...
#append to the confidences
confidences = np.append(confidences, np.unique(fd_conf_per_patient['RightCancer'].values))
confidences = np.unique(confidences)
#add 0 and 1 to the confidences
confidences = np.append([0], confidences)
...
for conf in confidences:
    ...
    if conf == 0.120:
        ld_hi_sens = np.mean(ld_conf_res, axis=0)
        fd_hi_sens = np.mean(fd_conf_res, axis=0)
    elif conf == 0.550:
        ld_hi_acc = np.mean(ld_conf_res, axis=0)
        fd_hi_acc = np.mean(fd_conf_res, axis=0)
    elif conf == 0.994:
        ld_hi_spec = np.mean(ld_conf_res, axis=0)
        fd_hi_spec = np.mean(fd_conf_res, axis=0)
...
#append 0 sens and 100 spec to the begining of the arrays
ld_means = np.append([[1, 0]], ld_means, axis=0)
ld_stds = np.append([[0, 0]], ld_stds, axis=0)
fd_means = np.append([[1, 0]], fd_means, axis=0)
fd_stds = np.append([[0, 0]], fd_stds, axis=0)
...
ax.plot(fd_means[:,1]*100, fd_means[:,0]*100, color='black', label='Full dose AI (AUC = {:.3f})'.format(fd_aucs*-1))
ax.plot(ld_means[:,1]*100, ld_means[:,0]*100, color='black', linestyle='dashed', label='Low dose AI (AUC = {:.3f})'.format(-1*ld_aucs))
#scatter plot operating points - solid for fd, dashed for ld
ax.scatter([fd_hi_sens[1]*100, fd_hi_acc[1]*100, fd_hi_spec[1]*100], [fd_hi_sens[0]*100, fd_hi_acc[0]*100, fd_hi_spec[0]*100], color='red', s=50, label='Full dose AI operating points')
ax.scatter([ld_hi_sens[1]*100, ld_hi_acc[1]*100, ld_hi_spec[1]*100], [ld_hi_sens[0]*100, ld_hi_acc[0]*100, ld_hi_spec[0]*100], s=50, label='Low dose AI operating points',
            linestyle='dashed',facecolors='none', edgecolors='red',linewidth=1.5)
...
# # plot the 95% confidence intervals with alpha = 0.2 for each curve using fill_between and 1.96*std
...
# ax.fill_between(fd_means[:,1]*100, (fd_means[:,0]-1.96*fd_stds[:,0])*100, (fd_means[:,0]+1.96*fd_stds[:,0])*100, alpha=0.2, color='black')
# ax.fill_between(ld_means[:,1]*100, (ld_means[:,0]-1.96*ld_stds[:,0])*100, (ld_means[:,0]+1.96*ld_stds[:,0])*100, alpha=0.2, color='black')

lt = ax.legend(loc='lower left', fontsize=16)
...
ax1.bar(['Large', 'Small'], [(45/59)*100, (14/59)*100],color=['palegoldenrod', 'lightsteelblue'], hatch=['\\', '/'],
        edgecolor='black', linewidth=2)
#place text of percentages on top of bars
for i, v in enumerate([(45/59)*100, (14/59)*100]):
    ax1.text(i, v + 1, str(round(v, 1))+'%', color='black', ha='center', fontsize=16,
                fontname='Helvetica')

ax1.set_ylabel('Percentage of tumours in ' +r"$\mathbf{Internal}$"+" dataset", fontsize=16,
                fontname='Helvetica')
...
plt.savefig('/Users/mcgoug01/Cambridge University Dropbox/William McGough/nat_paper/internal_bootstrap.png')
plt.show()
