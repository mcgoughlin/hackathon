...
meta_folder = '/Users/mcgoug01/Cambridge University Dropbox/William McGough/AIvsRad/AI/predictions_real/fulldose_ROC'
...
truth_csv_fp = '/Users/mcgoug01/Cambridge University Dropbox/William McGough/AIvsRad/Bill/FD/internaltest_metadata_real.csv'
sizes_fp ='/Users/mcgoug01/Cambridge University Dropbox/William McGough/AIvsRad/AI/predictions_real/fulldose_ROC/observations.csv'
...
for name,pred_csv_fp in zip(names,folders):
    ...
    # remove cases in pred and truth where size in sizes is greater than sphere with radius 15
    sphere_size = 4/3*np.pi*15**3
    ids = np.unique(sizes[sizes['Size'] > sphere_size].ID.values)
    pred = pred[~pred['OriginalID'].isin(ids)]
    truth = truth[~truth['OriginalID'].isin(ids)]
...
plt.title('ROC Curve on Full Dose Internal Test (<15mm diam)'.format(auc),fontdict={'fontsize':18})
